...
// LOTE 11: +stageRanks (solo guarda rangos válidos S/A/B/C/R)
...
// Agregar al final de sanitizeSave(), antes del return:
if (s.stageRanks && typeof s.stageRanks === 'object') {
  const validRanks = ['S', 'A', 'B', 'C', 'R'];
  o.stageRanks = {};
  Object.entries(s.stageRanks).forEach(([stage, rank]) => {
    const stageNum = parseInt(stage);
    if (!isNaN(stageNum) && stageNum >= 1 && stageNum <= 9999 && validRanks.includes(rank)) {
      o.stageRanks[stageNum] = rank;
    }
  });
} else {
  o.stageRanks = {};
