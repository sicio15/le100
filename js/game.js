...
function spawnEnemy() {
    ...
    enemies.push({ hp, max: hp, slot, x: W + 60, atkT: 1, boss: false,
        hue: (S.stage * 25) % 360, size: 1 });
}
function spawnBoss() {
    const hp = eHP(S.stage) * 10;
    enemies.push({ hp, max: hp, slot: 1, x: W + 80, atkT: 1, boss: true,
        hue: (S.stage * 25) % 360, size: 2.2 });
    bossT = 30;
...
function draw() {
    ...
    ctx.fillRect(0, groundY() + 26, W, H);
    ctx.fillStyle = '#28402c';
    ctx.fillRect(0, groundY() + 22, W, 8);
    ...
    const lx = hero.lunge * 22;
    ctx.save();
    ctx.translate(hx + lx, gy + bob * 0.4);
    ...
    if (heroReady) {
        ...
        ctx.fillStyle = '#fff';
        ctx.beginPath(); ctx.arc(28, -40, 6, 0, TAU); ctx.fill();
        ctx.fillStyle = '#111';
        ctx.beginPath(); ctx.arc(30, -40, 3, 0, TAU); ctx.fill();
    }
    ctx.restore();
    ...
    enemies.forEach(e => {
        ...
        // ojo
        ctx.fillStyle = '#fff';
        ctx.beginPath(); ctx.arc(-24, -27, 5, 0, TAU); ctx.fill();
        ctx.fillStyle = '#111';
        ctx.beginPath(); ctx.arc(-25, -27, 2.5, 0, TAU); ctx.fill();
        ctx.restore();
        // barra de vida
        const bw = 50 * s;
        ctx.fillStyle = 'rgba(0,0,0,.6)';
        ctx.fillRect(ex - bw/2, ey - 60 * s, bw, 6);
        ctx.fillStyle = e.boss ? '#ff4757' : '#7bed9f';
        ctx.fillRect(ex - bw/2, ey - 60 * s, bw * Math.max(0, e.hp / e.max), 6);
        if (e.boss) {
            ctx.font = '20px serif'; ctx.textAlign = 'center';
            ctx.fillText('👑', ex, ey - 70 * s);
        }
    });
...
Object.keys(UPDEF).forEach(k => {
    ...
    c.className = 'ucard';
    c.innerHTML = `<div class="un">${UPDEF[k].icon} ${UPDEF[k].name} <span class="ul" id="lv_${k}">Nv 0</span></div>
