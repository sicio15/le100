'use strict';
/* ===== FASE 4: Torre + Roguelike + QoL equipo =====
   NOTA: gearScore/gearPower/itemPower viven en store.js. Acá NO se redeclarán. */
...
function rollChoices(){ const pool=RL_BUFFS.slice(),out=[]; for(let i=0;i<3&&pool.length;i++) out.push(pool.splice(Math.random()*pool.length|0,1)[0]); return out; }
...
function pickBuff(i){
    const c=RL.choices[i]; c.f(RL.b,RL);
    ...
    let taken=eatk*8*(win?0.5:1); taken*=Math.max(0.4,1-(RL.b.hp-1)*0.4);
    RL.hp-=taken; if(RL.b.ls>0) RL.hp=Math.min(RL.maxHp,RL.hp+our*RL.b.ls);
    const msgEl=$('rogueMsg');
    if(RL.hp<=0){ if(msgEl)msgEl.textContent='💀 Caíste en la sala '+RL.room+'…'; setTimeout(()=>endRun(false),700); return; }
    if(!win){ if(msgEl)msgEl.textContent='💀 El guardián te superó…'; setTimeout(()=>endRun(false),700); return; }
    if(RL.room>=8){ if(msgEl)msgEl.textContent='🏆 ¡SOTOBOSQUE COMPLETO!'; setTimeout(()=>endRun(true),700); return; }
...
function endRun(cleared){
    ...
    const g=goldKill(S.best+5)*Math.max(1,rooms)*3; S.gold+=g;
    let msg='🌀 Run: '+rooms+'/8 salas · +'+fmt(g)+' 🪙';
    if(rooms>=3) dropItem(Math.floor(rooms/3)-1);
    if(cleared){ S.adn++; msg+=' · ¡PERFECTO! +1🧬'; }
    persist(); toast(msg); if(cleared)Audio.SFX.levelup();else Audio.SFX.death();
