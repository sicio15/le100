...
/* ===== FASE 4: Torre + Roguelike + QoL equipo =====
   NOTA: gearPower/itemPower viven en store.js. Acá NO se redeclarán. */
...
function pickBuff(i) {
  if (!RL || RL.over) return;                      /* FIX: ignora clicks tras el fin de la run */
  const c = RL.choices[i]; if (!c) return;         /* FIX: índice inválido */
  c.f(RL.b, RL);
