...
/* OPTIMIZACIÓN: elementos del hot-path (uiTick 10Hz) cacheados 1 sola vez.
   Antes: ~27 getElementById por tick (≈270/s). Ahora: 0. */
const EL = {};
...
(function buildUps() {
  ...
  Object.keys(UPDEF).forEach(k => {
    ...
    upLvs[k] = c.querySelector('.ul');   /* OPTIMIZACIÓN: span de nivel cacheado */
    upBtns[k].onclick = () => {
      const co = cost(k);
      if (S.gold >= co) {
        S.gold -= co; S.ups[k]++;
...
function uiTick() {
  ...
  for (const k in UPDEF) {
    ...
    const co = cost(k);   /* OPTIMIZACIÓN: 1 cost() por mejora y tick (antes 2) */
    b.textContent = '🪙 ' + fmt(co);
    b.disabled = S.gold < co;
  }
  /* OPTIMIZACIÓN: dots a 2Hz en vez de 10Hz (hasBetterGear/ACH.some no necesitan más) */
  if (++dotAcc >= 5) {
    dotAcc = 0;
