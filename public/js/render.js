// ===== Dibujo optimizado (sin shadowBlur, pasadas batcheadas) =====
...
function drawCentipede(c, camX, camY, t) {
    ...
    // Auras (halos baratos, sin shadowBlur)
    if (ab.poison && ab.poison.active > 0) {
        ctx.fillStyle = 'rgba(160,32,240,.22)';
        ctx.beginPath(); ctx.arc(sx, sy, 55, 0, TAU); ctx.fill();
        ctx.fillStyle = 'rgba(160,32,240,.25)';
    ...
    // PATAS: todas en un solo path (1 stroke)
    ctx.lineCap = 'round';
    ...
    // CUERPO: 3 pasadas por color (franjas + acentos)
    const cols = [
        `hsl(${hue},88%,62%)`,
        `hsl(${hue},85%,47%)`,
        `hsl(${(hue+35)%360},90%,60%)`
    ];
    for (let g = 0; g < 3; g++) {
        ctx.fillStyle = cols[g];
        ctx.beginPath();
        for (let i = segs.length-1; i >= 0; i--) {
            const kind = i % 6 === 0 ? 2 : (i % 2);
            if (kind !== g) continue;
            const s = segs[i];
...
function drawFood(f, camX, camY, t) {
    ...
    if (f.type === 'big') {
        ...
        ctx.beginPath(); ctx.arc(x, y, 16+pulse, 0, TAU); ctx.fill();
        ...
        ctx.beginPath(); ctx.arc(x, y, 12+pulse, 0, TAU); ctx.fill();
        ...
        ctx.fillStyle = FOOD_PAL[f.id % FOOD_PAL.length];
...
function drawGrid(camX, camY) {
    ...
    ctx.stroke();
    // Borde con doble trazo (sin shadowBlur)
    ctx.strokeStyle = 'rgba(255,60,60,.2)'; ctx.lineWidth = 16;
    ctx.strokeRect(-camX, -camY, G.worldSize, G.worldSize);
    ctx.strokeStyle = 'rgba(255,60,60,.9)'; ctx.lineWidth = 4;
    ctx.strokeRect(-camX, -camY, G.worldSize, G.worldSize);
}
